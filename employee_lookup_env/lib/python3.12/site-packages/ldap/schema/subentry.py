...
class SubSchema:
    ...
    def tree(self, schema_element_class, schema_element_filters=None):
        ...
        for se_oid in avail_se:
            ...
            if se_obj.__class__ != schema_element_class:
                ...
                continue
            assert (
                se_obj.__class__ == schema_element_class
                ...
                if raise_keyerror:
                    raise KeyError(
                        "No registered {}-OID for nameoroid {}".format(
                            se_class.__name__, repr(nameoroid_stripped)
                        )
                    )
                else:
    ...
    def get_obj(self, se_class, nameoroid, default=None, raise_keyerror=0):
        ...
        try:
            ...
            if raise_keyerror:
                raise KeyError(
                    "No ldap.schema.{} instance with nameoroid {} and se_oid {}".format(
    ...
    def attribute_types(
        ...
        object_class_oids = [self.getoid(ObjectClass, o) for o in object_class_list]
        # Initialize
        oid_cache = {}
        ...
        while object_class_oids:
            ...
            for a in object_class.must:
                ...
                r_must[se_oid] = self.get_obj(
                    AttributeType, se_oid, raise_keyerror=raise_keyerror
                )
            ...
            object_class_oids.extend(
                [self.getoid(ObjectClass, o) for o in object_class.sup]
            )
        ...
        if not ignore_dit_content_rule:
            ...
            if structural_oc:
                ...
                try:
                    dit_content_rule = self.get_obj(
                        DITContentRule, structural_oc, raise_keyerror=1
                    )
                except KeyError:
                    ...
                    for a in dit_content_rule.must:
                        se_oid = self.getoid(
                            AttributeType, a, raise_keyerror=raise_keyerror
                        )
                        r_must[se_oid] = self.get_obj(
                            AttributeType, se_oid, raise_keyerror=raise_keyerror
                        )
                ...
                for a in list(l):
                    for afk, afv in attr_type_filter:
                        try:
                            ...
                            if raise_keyerror:
                                raise KeyError(
                                    "No attribute type found in sub schema by name %s"
                                    % (a)
                                )
