...
class SimpleLDAPObject:
    ...
    def __init__(
        ...
        if fileno is not None:
            ...
            self._l = ldap.functions._ldap_function_call(
                ldap._ldap_module_lock, _ldap.initialize, uri
            )
        self.timeout = -1
            ...
            if self._trace_level >= 1:
                ...
                if self._trace_level >= 9:
                    traceback.print_stack(
                        limit=self._trace_stack_limit, file=self._trace_file
                    )
            ...
            try:
                ...
                if __debug__ and self._trace_level >= 2:
                    if func.__name__ != "unbind_ext":
                        diagnostic_message_success = self._l.get_option(
    ...
    def __getattr__(self, name):
        if name in self.CLASSATTR_OPTION_MAPPING:
            ...
            raise AttributeError(
                "{} has no attribute {}".format(self.__class__.__name__, repr(name))
            )
    ...
    def bind(self, who, cred, method=ldap.AUTH_SIMPLE):
        ...
        ), "Only simple bind supported in LDAPObject.bind()"
    ...
    def sasl_external_bind_s(
        ...
        """
        self.sasl_non_interactive_bind_s(
    ...
    def extop_result(self, msgid=ldap.RES_ANY, all=1, timeout=None):
        resulttype, msg, msgid, respctrls, respoid, respvalue = self.result4(
            msgid,
            all=1,
    ...
    def extop_s(
        self, extreq, serverctrls=None, clientctrls=None, extop_resp_class=None
    ):
    ...
    def passwd_s(
        ...
        if respoid != PasswordModifyResponse.responseName:
            raise ldap.PROTOCOL_ERROR(
                "Unexpected OID %s in extended response!" % respoid
            )
        if extract_newpw and respvalue:
            respvalue = PasswordModifyResponse(
                PasswordModifyResponse.responseName, respvalue
            )
    ...
    def result4(
        ...
        if ldap_result is None:
            ...
            if add_ctrls:
                resp_data = [
                    (t, r, DecodeControlTuples(c, resp_ctrl_classes))
    ...
    def search_st(
        self, base, scope, filterstr=None, attrlist=None, attrsonly=0, timeout=-1
    ):
        return self.search_ext_s(
            base, scope, filterstr, attrlist, attrsonly, None, None, timeout
        )
    ...
    def read_subschemasubentry_s(self, subschemasubentry_dn, attrs=None):
        ...
        try:
            subschemasubentry = self.read_s(
                subschemasubentry_dn, filterstr=filterstr, attrlist=attrs
            )
    ...
    def find_unique_entry(
        ...
        if len(r) != 1:
            raise NO_UNIQUE_ENTRY(
                "No or non-unique search result for %s" % (repr(filterstr))
            )
        return r[0]
    ...
    def get_naming_contexts(self):
        ...
        """
        name = "namingContexts"
        return self.read_rootdse_s(attrlist=[name]).get(name, [])
...
class ReconnectLDAPObject(SimpleLDAPObject):
    ...
    * retry_delay: specifies the time in seconds between reconnect attempts.
    ...
    def __init__(
        ...
        self._reconnect_lock = ldap.LDAPLock(
            desc="reconnect lock within %s" % (repr(self))
        )
    ...
    def __getstate__(self):
        ...
        state = {
            k: v for k, v in self.__dict__.items() if k not in self.__transient_attrs__
        }
        state["_last_bind"] = (
        ...
        try:
            ...
            while reconnect_counter:
                ...
                try:
                    ...
                    if __debug__ and self._trace_level >= 1:
                        self._trace_file.write(
                            "*** {} reconnect to {} successful => repeat last operation\n".format(
                                counter_text, uri
                            )
                        )
    ...
    def _apply_method_s(self, func, *args, **kwargs):
        if not hasattr(self, "_l"):
            self.reconnect(
                self._uri, retry_max=self._retry_max, retry_delay=self._retry_delay
            )
        try:
