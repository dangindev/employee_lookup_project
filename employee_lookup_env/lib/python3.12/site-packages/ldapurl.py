...
class LDAPUrlExtension:
    ...
    def unparse(self):
        if self.exvalue is None:
            ...
            return "{}{}={}".format(
                "!" * (self.critical > 0), self.extype, quote(self.exvalue or "")
            )
...
class LDAPUrlExtensions(MutableMapping):
    ...
    def __setitem__(self, name, value):
        ...
        if name != value.extype:
            raise ValueError(
                "key {!r} does not match extension type {!r}".format(name, value.extype)
            )
...
class LDAPUrl:
    ...
    def _parse(self, ldap_url):
        ...
        if not isLDAPUrl(ldap_url):
            raise ValueError(
                "Value %s for ldap_url does not seem to be a LDAP URL."
                % (repr(ldap_url))
            )
    ...
    def unparse(self):
        ...
        if self.urlscheme == "ldapi":
            ...
            hostport = self.hostport
        ldap_url = "{}://{}/{}?{}?{}?{}".format(
            self.urlscheme, hostport, dn, attrs_str, scope_str, filterstr
        )
        if self.extensions:
    ...
    def htmlHREF(self, urlPrefix="", hrefText=None, hrefTarget=None):
        ...
        if hrefTarget is None:
            ...
            if not isinstance(hrefTarget, str):
                raise TypeError(
                    "hrefTarget must be str, not " + type(hrefTarget).__name__
                )
    ...
    def __getattr__(self, name):
        if name in self.attr2extype:
            ...
            if (
                ...
                and not self.extensions[extype].exvalue is None
            ):
